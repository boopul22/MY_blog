...
const DashboardPage: React.FC = () => {
    ...
    const StatCard = ({ title, value, icon }: { title: string, value: number, icon: React.ReactNode }) => (
        <div className="bg-white dark:bg-medium-dark p-6 rounded-lg shadow-md flex items-center">
            <div className="p-3 bg-primary/20 rounded-full mr-4 text-primary">
                {icon}
            </div>
            <div>
        ...
        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white dark:bg-medium-dark p-6 rounded-lg shadow-md">
                    <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-200">Recent Posts</h3>
                        ...
                        {posts.slice(0, 5).map(post => (
                            <li key={post.id} className="flex justify-between items-center">
                                <div>
                                    ...
                                    <p className={`text-xs font-semibold ${post.status === 'published' ? 'text-green-500' : 'text-yellow-500'}`}>{post.status}</p>
                                </div>
